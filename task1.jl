...
function plot_rooms(df_full::AbstractDataFrame)
    ...
    @df df_rooms plot(:Rooms, :count, seriestype = :bar, xlabel = "Rooms", ylabel = "Property Count", label = "", yscale=:log10, xticks=1:20, plot_title = "Room Distribution")
end
...
function plot_price(df_full::AbstractDataFrame)
    ...
    @df df_price plot(:Price, :cum_rownum, xscale=:log10, xticks = ([100000, 1000000, 10000000], ["\$10,000", "\$100,000", "\$1,000,000"]), yformatter = :plain, label="", xlabel = "Property Sale Price (\$AUD)", ylabel = "Num. of Properties Sold For More", plot_title = "Cumulative Sale Price")#, yticks = 1:10^8:5*10^8)
end
...
function plot_distance(df_full::AbstractDataFrame)
    ...
    # We will use a cumulative plot. It feels appropriate, considering we are using continuous data of a large volume.
    @df df_distance plot(:Distance, :cum_rownum, xticks=0:10:50, yformatter = :plain, label="", xlabel="Distance From CBD (km)", ylabel="Num. of Properties Further From CBD", plot_title="Cumulative CBD Distance")#, xticks = ([100000, 1000000, 10000000], ["\$10,000", "\$100,000", "\$1,000,000"]), yformatter = :plain, label="", xlabel = "Property Value (\$AUD)", ylabel = "Num. of Properties Sold At or Below", plot_title = "Cumulative Sale Price")
end
...
function plot_method(df_full::AbstractDataFrame)
    ...
    # Discrete data? Bar graph.
    @df df_method plot(:Method, :count, seriestype = :bar, xlabel = "Sale Method", ylabel = "Property Count", label = "", yformatter = :plain, plot_title = "Sale Method")
end
...
function plot_landsize(df_full::AbstractDataFrame)
    ...
    @df df_landsize plot(:Landsize_Grouped, :count, seriestype = :line, xticks=(10:10:56, adjustment_func.(10:10:56)), label="", tickfonthalign=:center, xlabel="Landsize (m²)", ylabel="Number of Properties", plot_title="Property Landsizes")
end


#plot_price(df_full)
plot_distance(df_full)
#plot_method(df_full)
#plot_landsize(df_full)